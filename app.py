...
from models import ContentItem, TestOption, TestQuestion, TestSession
...
class TestApp(tk.Tk):
    ...
    def _extract_tests(self) -> None:
        ...
        log.info("User selected file: %s", path)
        if not path:
        ...
        extractor.cleanup()
        log.info("Extraction finished. Tests=%d Output=%s", len(tests), output_path)
